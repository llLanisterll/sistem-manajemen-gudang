<nav x-data="{ open: false }" class="bg-white/90 backdrop-blur-md border-b border-gray-100 sticky top-0 z-40">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(Auth::user()->role === 'admin')
                        ...
                        <x-nav-link :href="route('admin.categories.index')" :active="request()->routeIs('admin.categories.*')">{{ __('Categories') }}</x-nav-link>
                        <x-nav-link :href="route('admin.products.index')" :active="request()->routeIs('admin.products.*')">{{ __('Products') }}</x-nav-link>

                    {{-- MENU MANAGER --}}
                    @elseif(Auth::user()->role === 'manager')
                        <x-nav-link :href="route('manager.dashboard')" :active="request()->routeIs('manager.dashboard')">{{ __('Dashboard') }}</x-nav-link>
                        <x-nav-link :href="route('manager.transactions.index')" :active="request()->routeIs('manager.transactions.*')">{{ __('Transactions') }}</x-nav-link>
                        <x-nav-link :href="route('manager.restock.index')" :active="request()->routeIs('manager.restock.*')">{{ __('Restock Order') }}</x-nav-link>

                    {{-- MENU STAFF --}}
                    @elseif(Auth::user()->role === 'staff')
                        <x-nav-link :href="route('staff.dashboard')" :active="request()->routeIs('staff.dashboard')">{{ __('Dashboard') }}</x-nav-link>
                        <x-nav-link :href="route('staff.transactions.index')" :active="request()->routeIs('staff.transactions.*')">{{ __('Transactions') }}</x-nav-link>

                    {{-- MENU SUPPLIER --}}
                    @elseif(Auth::user()->role === 'supplier')
                        <x-nav-link :href="route('supplier.dashboard')" :active="request()->routeIs('supplier.dashboard')">{{ __('Dashboard') }}</x-nav-link>
                        <x-nav-link :href="route('supplier.restock.index')" :active="request()->routeIs('supplier.restock.*')">{{ __('Pesanan Masuk') }}</x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-white border-b">
        <div class="pt-2 pb-3 space-y-1">
            {{-- Copy logic menu mobile dari file sebelumnya jika perlu, atau gunakan component responsive link --}}
             @if(Auth::user()->role === 'admin')
